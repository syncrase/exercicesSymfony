forms
	Exercices avec les formulaires twig
.
server
	Configuration pour que le projet s'exécute sur serveur (wamp, easyPhp, ...)
.
files
	Configuration pour pouvoir (down|up)load des fichiers sur le serveur
.
	(( tiddlywiki ))
		Configuration pour pouvoir sauvegarder le TiddlyWiki sur le serveur
	.
debug
	Configuration pour pouvoir debugger l'application pas à pas
.
data-provider-pool
	Configuration de manière à avoir plusieurs data providers (ORM (MySQL, mongo) + à la main)
.
best-practices
	Configuration respectant toutes les best practices et expliquant en quoi ce sont les meilleures
.
dynamic-database
	Configuration qui permet de créer en runtime des tables des tables de les compléter, les modifier, les supprimer, ...
.
mind-map
	Permet la construction d'une mind map
.
chronologie
	Permet la construction d'une chronologie
.
cartographie
	Permet la construction d'une carte géographique personnalisée
.
web-scrolling
	Permet le scrolling d'information sur internet. Test de plusieurs APIs
.
encyclopedie
	Gestion de mots avec éthymologie, fréquence, champ lexical, subsomption, lexique/glossaire (ex. plantes) notation par ordre d'importance, ...
.
plantes
	Classification des plantes, toutes les info théoriques + pratiques (lieux de présence, recettes, risque de confusion) les nécessités du sols (ph, composition, acidité) et les implications sur l'environnement (augmentation de l'acidité)
	Les symbioses (champignon), mode de reproduction
	photos de l'écorce fonction des âges, des fruits, des feuilles fonction de la saison, ...
	les types de racine
	outils d'identification
	stades phénologiques
.