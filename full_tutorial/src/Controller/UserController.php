...
//        $this->cache = $cache;
     ...
     */
    public function showUser($showUserParam1, EntityManagerInterface $em){
        $comments = [
...
//EOF;
        ...
        $user = $repository->findOneBy(['email' => 'email@email.email']);
        if (!$user) {
            throw $this->createNotFoundException(sprintf('No user for email "%s"', $user));
        }
//        $userContent = $this->markdownHelper->parse($user->getDescription());
